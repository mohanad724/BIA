...
class ProductionLine:
    def __init__(
        self, tasks_ids: list[int], total_time, average_time: float, idx: int
    ) -> None:
    ...
    def write(self, writable_file: TextIO) -> None:
        writable_file.write(
            f"{self.idx} : {self.tasks_ids} Total Time=({self.total_time}), dif={round(self.diff, 2)} \n"
        )
...
def generate_all_sorted_tasks(
    links: dict[int, list[int]],
    need: dict[int, int],
    all_sorted_tasks: list[list[int]],
    ...
    for cur in links.keys():
        if not need[cur] and cur not in sorted_tasks:
            ...
            generate_all_sorted_tasks(
                links, new_need, all_sorted_tasks, new_sorted_tasks
            )
...
def get_all_solutions(
    tasks_times: dict[int, int], links: dict[int, list[int]], steps: int
) -> list[Solution]:
    ...
    for sorted_tasks in all_sorted_tasks:
        for cut in all_cuts:
            ...
            for i in range(len(cut) - 1):
                ...
                cur_total_time = sum(
                    [tasks_times[task_id] for task_id in cur_tasks_ids], dtype=int
                )
